...
- 当前分支状态：查看 `git status --short --branch`。只读分析本身不要改动用户工作区；但如果本地 fork 落后于上游仓库，必须先按“0. 同步 fork 与上游仓库”完成同步，再继续分析。
...
- 报告中的每个候选 Issue 和被排除的关键 Issue 都必须包含可点击的 Issue 链接，便于用户直接打开复核。
...
在所有仓库阅读、Issue 筛选、源码扫描和报告生成之前，先确认当前目标仓库是否是 fork，且是否落后于上游仓库。

1. 查看远程与分支状态：
   - 执行 `git remote -v` 和 `git status --short --branch`。
   - 优先把 `upstream` 视为上游仓库，把 `origin` 视为用户 fork。
   - 如果没有 `upstream`，但 `origin` 是 GitHub fork，优先用 `gh repo view --json parent,defaultBranchRef` 或用户提供的仓库地址确认 parent/source 仓库；不能可靠确认时，先向用户说明并询问上游仓库地址，不要继续基于陈旧 fork 做分析。
2. 获取最新远程引用：
   - 至少执行 `git fetch origin`。
   - 确认有上游后执行 `git fetch upstream`。
3. 确认对比基准：
   - 优先使用上游默认分支；若当前分支跟踪的是明确的贡献基线分支，则使用该基线分支。
   - 用 `git rev-list --left-right --count <local-base>...upstream/<base>` 或等价命令判断本地 fork 基线是否落后。
4. 如果本地 fork 落后于上游：
   - 先检查工作区是否干净。若存在未提交改动，不要自动 stash、提交或覆盖，先提示用户需要清理/保存改动后再同步。
   - 在干净工作区中，优先执行快进同步：`git checkout <base>`，`git merge --ff-only upstream/<base>`，然后按需要执行 `git push origin <base>` 更新 fork。
   - 如果无法快进，停止并说明分叉原因与建议处理方式，不要强制 rebase、reset 或覆盖历史。
5. 同步完成后，再次执行 `git status --short --branch`，确认本地 fork 基线不再落后，然后继续后续步骤。

如果用户只提供远程仓库 URL、没有本地 fork 路径，则无法执行本地同步；此时必须在报告中明确标注“未验证本地 fork 同步状态”，并优先基于上游仓库当前状态分析。
...
访问仓库 Issue 列表，优先使用可用的 GitHub 工具、`gh` CLI 或网页。对进入候选池的 Issue，必须获取 Issue 详情、assignees、comments，并在工具支持时补充 timeline / linked pull requests 信息；不能只看列表页摘要。只推荐同时满足这些条件的 Issue：
...
- 评论列表、timeline 或 linked pull requests 中没有已经关联到该 Issue 的 PR。排除信号包括但不限于：评论中出现 `https://github.com/{owner}/{repo}/pull/{number}`、`/pull/{number}`、`PR #{number}`、`pull request #{number}`、`fixes #{issue}` / `closes #{issue}` 指向的 PR，或 timeline 中已有 cross-referenced / connected PR。
- 描述清晰，有期望行为、复现步骤、验收条件或维护者认可的问题边界。
- 预计改动范围在 1-3 个核心文件内，不依赖大规模重构或维护者私有上下文。

若使用 `gh` CLI，优先组合 `gh issue list --json number,title,url,labels,assignees,updatedAt,comments`、`gh issue view <number> --json number,title,url,body,labels,assignees,comments` 和必要的 `gh api repos/{owner}/{repo}/issues/<number>/timeline` 来确认评论与 PR 关联状态。
