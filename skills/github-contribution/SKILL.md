...
阶段推进原则：执行完 `/find-contribution-prs` 后必须暂停，等待用户从分析报告中选择并确认一个 PR 方向。即使用户在最初请求中说“全流程完成”或“你来安排”，也只能先给出推荐排序和理由，不能在没有用户明确确认具体方向的情况下进入 `/design-pr-technical-plan`。
...
使用 `references/workflow-state-template.md` 维护阶段状态。工作流状态和后续阶段文档都必须落盘到目标开源项目根目录的 `tmp/` 目录，而不是只在聊天回复中输出。

### 文档落盘规则

- 先确定目标开源项目根目录 `OPEN_SOURCE_ROOT`：优先使用 `LOCAL_PATH`；若当前目录就是目标仓库，则使用当前目录；不要把本 skills 仓库误当成目标仓库，除非用户正在贡献的目标就是本仓库。
- 在写任何阶段文档前，确保 `OPEN_SOURCE_ROOT/tmp/` 存在。
- 默认文档路径：
  - `OPEN_SOURCE_ROOT/tmp/workflow-state.md`
  - `OPEN_SOURCE_ROOT/tmp/contribution-analysis.md`
  - `OPEN_SOURCE_ROOT/tmp/technical-plan.md`
  - `OPEN_SOURCE_ROOT/tmp/interview-narrative.md`
  - `OPEN_SOURCE_ROOT/tmp/implementation-report.md`
  - `OPEN_SOURCE_ROOT/tmp/pr-description.md`
- 如果用户只指定文件名，仍写入 `OPEN_SOURCE_ROOT/tmp/`；只有用户明确指定其他目录时，才按用户指定目录执行。
- 聊天回复只保留摘要、关键结论、验证结果和已写入文件的绝对路径，不要把完整长文档只放在聊天中。
- 这些 `tmp/` 文档默认是本地工作产物，不要纳入 commit 或 PR，除非用户明确要求。
...
- 输出分析报告后必须暂停，等待用户明确选择并确认一个 PR 方向，再进入下一阶段。
- 可以从 Top 3 中推荐一个最稳妥方向，并说明推荐理由；但推荐不等于确认，不能替用户自动进入技术方案设计。
- 如果用户在运行 `/find-contribution-prs` 之前已经明确指定 PR 方向，仍需在分析报告完成后复述该方向并请求一次最终确认。
...
- 默认写入 `OPEN_SOURCE_ROOT/tmp/technical-plan.md` 和 `OPEN_SOURCE_ROOT/tmp/interview-narrative.md`。
...
- 默认写入 `OPEN_SOURCE_ROOT/tmp/implementation-report.md`。
...
- PR 描述草稿默认写入 `OPEN_SOURCE_ROOT/tmp/pr-description.md`；如果已创建 PR，聊天回复中附 PR 链接和摘要即可。
